...
class _SecurePaymentScreenState extends State<SecurePaymentScreen> {
  ...
  Future<void> _startPayment() async {
    if (widget.amount <= 0) {
      _showError('This plan does not require payment.');
      return;
    }
    ...
    try {
      ...
      final initialized = await provider.initializePayment(
      ...
      if (!initialized) {
        _showError(provider.errorMessage ?? 'Unable to initialize payment.');
        return;
      }

      final clientSecret = provider.clientSecret;

      if (clientSecret == null || clientSecret.isEmpty) {
        _showError('Stripe payment information was not received.');
        return;
      }
      ...
      await Stripe.instance.initPaymentSheet(
        paymentSheetParameters: SetupPaymentSheetParameters(
          ...
          customerEphemeralKeySecret: provider.ephemeralKey,
          merchantDisplayName: 'BookVerse',
      ...
      await provider.markPaymentCompleted();

      if (!mounted) return;

      _showConfirmingDialog();
      provider.listenForConfirmation(provider.paymentId!);
    } on StripeException catch (e) {
      if (mounted) {
        _showError(e.error.localizedMessage ?? 'Payment was cancelled or failed.');
      }
      await provider.markPaymentFailed();
    } catch (e) {
      if (mounted) {
  ...
  // ============================================================
  // CONFIRMING DIALOG — webhook confirmation ka wait karta hai
  // ============================================================
      ...
      builder: (dialogContext) {
        return Consumer<PaymentProvider>(
          builder: (context, provider, _) {
            if (provider.verificationStatus == PaymentVerificationStatus.confirmed) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                Navigator.of(dialogContext).pop();
                _showPaymentSuccessDialog();
              });
            } else if (provider.verificationStatus == PaymentVerificationStatus.failed ||
                provider.verificationStatus == PaymentVerificationStatus.timeout) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                Navigator.of(dialogContext).pop();
                _showError(
                  provider.verificationStatus == PaymentVerificationStatus.timeout
                      ? 'Payment confirm hone mein waqt lag raha hai, thodi dair mein dobara check karein.'
                      : 'Payment confirm nahi ho saka.',
                );
              });
            }
            return const PaymentConfirmingDialog();
          },
        );
      },
    );
  ...
  Widget build(BuildContext context) {
    final amountText = '${widget.currency.toUpperCase()} ${widget.amount.toStringAsFixed(2)}';

    final subscriptionLabel =
        widget.billingCycle == 'monthly' ? '1 Month Subscription' : '1 Year Subscription';

    final priceSuffix = widget.billingCycle == 'monthly' ? 'per month' : 'per year';
                ...
                child: Column(
                  ...
                  children: [
                    PremiumPlanSummaryCard(
                      ...
                      perks: const ['∞ Unlimited Books', '🚫 No Ads'],
                      price: amountText,
                    ...
                    TotalAmountRow(amount: amountText),
